        ...
        unsafe {
            ...
            match *self {
                ...
                Self::CopyBufferToTexture {
                    ...
                    // Only set row length for uncompressed data when stride
                    // differs from width; redundant values trigger Firefox's
                    // slow defensive copy path. Compressed uploads use
                    // compressedTexSubImage, which takes no row length.
                    if !compressed && row_texels != size.width {
